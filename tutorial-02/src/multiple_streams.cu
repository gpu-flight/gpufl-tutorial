...
__global__ void stream_transform_kernel(const float* a,
                                        const float* b,
                                        float* c,
                                        int element_count,
                                        int iteration,
    ...
    try {
        ...
        for (int iteration = 0; iteration < kIterations; ++iteration) {
            for (int chunk = 0; chunk < kChunksPerIteration; ++chunk) {
                ...
                stream_transform_kernel<<<blocks, kThreadsPerBlock, 0, stream>>>(
        ...
        std::cout << "Multiple CUDA streams completed successfully" << std::endl;
